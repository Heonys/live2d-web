...
if (!values.input || !values.output) {
  throw new Error(
    ...
    + '--output docs/benchmarks/YYYY-MM-DD-<name>.md',
  )
}

const inputPath = path.resolve(values.input)
const outputPath = path.resolve(values.output)
const reportsDirectory = path.resolve('docs/benchmarks')
if (!outputPath.startsWith(`${reportsDirectory}${path.sep}`))
  throw new Error('Benchmark reports must be written under docs/benchmarks/.')
