...
Do not render an untrusted archive merely because its JSON parses. Inspect first, cap compressed and expanded sizes, reject traversal and unexpected external references, and serve approved files from a controlled origin. Server-side URL inspection needs outbound network limits as well as an allow-list.
