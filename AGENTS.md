...
26. **릴리스 전 CHANGELOG 헤더.** `release.yml`은 첫 `## ` 절을 릴리스
    ...
    제목 없는 노트가 올라간다.
27. **트래킹 품질은 실제 카메라로만 판정된다.** 초록 CI가 트래킹이 쓸 만하다는
    뜻이 아니다. 2026-08-25에 정지 초상 게이트가 전부 초록인 채로 네 결함이
    살아 있었다(얼굴을 너무 일찍 놓침, 놓치면 정면 복귀, 경계 프레임 튐, 물리
    앞이 아닌 뒤에 적용). 각도의 부호·크기는 합성 행렬 단위 테스트로 잠갔으니
    (`state.test.ts`) 나머지는 사람이 카메라 앞에서 본다.
28. **최대값은 유지값이 아니다.** 트래킹 수치를 읽을 때 `peak`만 보면 오진한다.
...
32. **파라미터의 중립은 `minimum`이 아니라 `defaultValue`다.** 공식 샘플 8종 중
    5종이 `ParamEyeLOpen` 최대값을 1.0 초과로 선언한다(Haru는 2.0). 트래킹
    테스트가 `getModelInfo()`를 `parameters` 없이 넘기면 폴백 표로 떨어지는데,
    그 표는 기본값이 늘 구동 방향 끝에 있어 **이 계열 결함이 드러날 수 없는
    유일한 형태**다. 실제 리그 범위를 쓰는 픽스처를 반드시 함께 둔다.

33. **`/assets/`의 404는 1년간 캐시된다.** `next.config.ts`의 캐시 규칙이 파일
    존재와 무관하게 적용되어 404 응답에도 `immutable`이 붙는다. 자산이 잠깐이라도
    빠지면 그것을 본 방문자는 새로고침으로도 복구되지 않는다. 재현 확인은 반드시
    시크릿 탭이나 웹사이트 데이터 삭제 후에 한다.
