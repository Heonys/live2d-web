...
export class CubismPhysics {
  ...
  public parse(physicsJson: ArrayBuffer, size: number): void {
    ...
    this._inputSlices.length = this._physicsRig.subRigCount;
    this._outputSlices.length = this._physicsRig.subRigCount;
    this._particleSlices.length = this._physicsRig.subRigCount;
    for (let i = 0; i < this._physicsRig.subRigCount; ++i) {
      const setting = this._physicsRig.settings[i];
      this._inputSlices[i] = this._physicsRig.inputs.slice(
        setting.baseInputIndex,
        setting.baseInputIndex + setting.inputCount
      );
      this._outputSlices[i] = this._physicsRig.outputs.slice(
        setting.baseOutputIndex,
        setting.baseOutputIndex + setting.outputCount
      );
      this._particleSlices[i] = this._physicsRig.particles.slice(
        setting.baseParticleIndex,
        setting.baseParticleIndex + setting.particleCount
      );
    }
  ...
  public stabilization(model: CubismModel): void {
    ...
    for (
      ...
      currentInputs = this._inputSlices[settingIndex];
      currentOutputs = this._outputSlices[settingIndex];
      currentParticles = this._particleSlices[settingIndex];
  ...
  public constructor() {
    ...
    this._inputSlices = new Array<CubismPhysicsInput[]>();
    this._outputSlices = new Array<CubismPhysicsOutput[]>();
    this._particleSlices = new Array<CubismPhysicsParticle[]>();
    this._currentRemainTime = 0.0;
   ...
   */
  public release(): void {
    this._inputSlices.length = 0;
    this._outputSlices.length = 0;
    this._particleSlices.length = 0;
  ...
  public initialize(): void {
    ...
    for (
      ...
      currentSetting = this._physicsRig.settings[settingIndex];
      strand = this._particleSlices[settingIndex];
  ...
  _inputSlices: Array<CubismPhysicsInput[]>; // サブリグごとの入力参照
  _outputSlices: Array<CubismPhysicsOutput[]>; // サブリグごとの出力参照
  _particleSlices: Array<CubismPhysicsParticle[]>; // サブリグごとのパーティクル参照
 ...
 * @param parameterIndex            Target parameter index.
...
function updateOutputParameterValue(
  ...
  if (weight >= 1.0) {
    parameterValue[parameterIndex] = value;
  } else {
    value =
      parameterValue[parameterIndex] * (1.0 - weight) + value * weight;
    parameterValue[parameterIndex] = value;
  }
}
