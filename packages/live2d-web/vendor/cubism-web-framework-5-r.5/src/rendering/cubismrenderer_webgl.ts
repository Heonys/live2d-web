...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public loadShaders(
    ...
    if (this.gl == null) {
      return Promise.reject(
        new Error(
          "'gl' is null. WebGLRenderingContext is required. Please call CubismRenderer_WebGL.startUp()."
        )
      );
    }
  ...
  public doDrawModel(shaderPath: string = null): void {
    ...
    // live2d-web owns this WebGL context and calls setRenderState immediately
    // before drawing. Reuse that known state instead of forcing two
    // synchronous driver queries on every frame.
    const lastFbo = this._renderStateFbo;
    const lastViewport = this._renderStateViewport;
  ...
  public setRenderState(fbo: WebGLFramebuffer, viewport: number[]): void {
    ...
    this._renderStateViewport[3] = viewport[3];
