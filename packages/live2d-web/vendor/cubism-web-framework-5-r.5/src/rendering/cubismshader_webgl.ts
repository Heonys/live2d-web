...
export class CubismShader_WebGL {
   ...
   * @return シェーダーのソースコード
   */
  private async loadShader(url: string, signal?: AbortSignal): Promise<string> {
    const fileName = url.slice(url.lastIndexOf('/') + 1);
    const bundledSource = this._shaderSources?.[fileName];
    if (bundledSource !== undefined) {
      if (bundledSource.trim().length == 0) {
        throw new Error(`Failed to load shader ${url}: empty source`);
      }
  ...
  public releaseShaderProgram(): void {
    ...
    for (let i = 0; i < this._shaderSets.length; i++) {
      ...
      if (shaderSet.shaderProgram) {
        programs.add(shaderSet.shaderProgram);
      }
      shaderSet.shaderProgram = null;
  ...
  public generateShaders(
    ...
    // シェーダーのソースの読み込み
    this._shaderLoadPromise = this.loadShaders(signal)
      .then(() => {
  ...
  public loadShaderProgram(
    ...
    if (!shaderProgram) {
      throw new Error('WebGL failed to create a shader program');
    }
    ...
    if (!linkStatus) {
      ...
      throw new Error(`Failed to link WebGL shader program: ${log ?? 'unknown error'}`);
    }
  ...
  public compileShaderSource(
    ...
    if (!status) {
      ...
      throw new Error(`${kind} shader compile error: ${log ?? 'unknown error'}`);
    }
  ...
  /** Supplies adapter-bundled shader sources when package assets are relocated. */
  public setShaderSources(shaderSources?: Readonly<Record<string, string>>): void {
  ...
  _shaderSources?: Readonly<Record<string, string>>;
}
...
export class CubismShaderManager_WebGL {
  ...
  /**
   * Releases shader programs owned by one WebGL context.
   * Added by the private live2d-web adapter so disposing one Stage does not
   * retain its context until every other Stage has also been released.
   */
