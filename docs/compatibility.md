...
상태 기준일: **2026-08-28**. 대상은 발행 준비를 마친 `0.7.0`이다. 이 문서는
...
| iOS 인앱 브라우저 | **부분 검증·비차단 (2026-08-28)** | Safari 본체만 두 모드 모두 정상이다. 아래 표 참조. 캐시 문제가 아님을 개인정보 보호 탭으로 확인했다. |
...
| 눈 감김 깊이 | **실측·기본값 유지 (2026-08-28)** | 눈높이의 맥 카메라에서 두 눈을 꽉 감아도 `ParamEyeLOpen`이 **0.20**에서 멈춘다. 역산하면 MediaPipe가 `eyeBlink`를 **약 0.8**까지만 보고한다. 계산상 `sensitivity.eyes` 1.25면 0에 닿고 실측으로 1.4에서 완전히 감긴다. **그런데 1.4는 평소 깜빡임까지 증폭해 눈이 부산스러워지고 좌우 비대칭이 두드러진다.** 감도는 곱셈이라 위쪽을 맞추면 아래쪽이 과해지는 것을 피할 수 없다. 방송 용도에서는 정확도보다 자연스러움이 중요하므로 기본값 1을 유지하고 Playground에 슬라이더와 실측 표시를 노출한다. 표본은 카메라 한 대다. |
| 자세 추정 크기 | 1대 측정 | 2026-08-28 iPhone에서 감도를 1로 두고 재면 `now` 열이 곧 실제 각도다. Y nod 15.7도, X turn -11.2도로 **pitch가 yaw보다 컸다.** "상하가 거의 반응하지 않는다"는 체감과 반대이므로 원시 추정의 문제가 아니다. 남은 후보는 리그의 상하 변형이 좌우보다 작게 그려진 경우이며 확인하지 않았다. |
