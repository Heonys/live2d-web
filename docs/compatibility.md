...
| 눈 감김 깊이 | **부분 검증** | 2026-08-28 실기 확인에서 두 눈을 완전히 감아도 모델이 끝까지 감기지 않는다. 파라미터 중립 수정으로 과다 개안과 앞 6분의 1 사각지대는 없어졌으므로 남은 원인은 다른 것이다. MediaPipe가 완전히 감은 눈에도 `eyeBlink`를 1.0까지 올리지 않는 쪽이 유력하다. `eyes` 감도 기본값이 1이라 증폭이 없다. 표본이 하나여서 기본값을 바꾸지 않았다. |
