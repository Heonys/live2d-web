...
- 중앙 FPS 74.63은 성능이 아니라 프레임 상한의 페이싱 특성이다. 측정에 쓴
  디스플레이의 주사율은 약 75Hz(간격 13.4ms)였다. 60 FPS 상한은 주사율이 60의
  정수배가 아닐 때 13.4ms 세 번과 26.8ms 한 번이 번갈아 나오는 톱니 패턴이
  된다. 20초에 1,201프레임으로 **평균은 정확히 60.0 FPS**이므로 상한 자체는
  의도대로 동작한다. 60Hz 디스플레이에서 측정한 08-15 스모크는 간격이 균일해
  59.88 FPS로 기록됐다.
- 이 톱니는 75Hz, 90Hz, 144Hz, 165Hz처럼 60의 정수배가 아닌 주사율에서 모두
  나타난다. 60Hz와 120Hz에서는 간격이 균일하다. 평균 프레임률은 어느
  주사율에서나 60 FPS로 유지된다.
