...
상태 기준일: **2026-08-27**(소비자 실측과 0.8 착수 조건). 기능 상태의
기준일은 2026-08-26이다. 현재 발행 버전은 `0.6.0`이다. Worker,
...
**아직 답하지 못하는 두 질문.** 기능 목록과 별개로, 기반 라이브러리에 오는 첫
두 질문에 답이 없다. **"내 모델이 되나"**는 검증이 공식 Hiyori 한 종뿐이라
VTube Studio 내보내기나 다른 리거 도구 출력에 대해 아무 말도 못 한다(G 지원
매트릭스). **"10분 안에 띄울 수 있나"**는 F가 검증 조건으로 적어두고 아직
실행하지 않았다. 둘 다 소비자를 기다리지 않는 작업이며, 소비자가 늘지 않는
이유가 기능 부족이 아니라 여기일 가능성이 있다.

**소비자 (2026-08-27 실측 정정).** 이전 판은 Livesona를 "중단 중"으로,
활성 소비자를 0으로 적었다. **둘 다 사실과 다르다.** Livesona는 최근 30일에
26커밋이 있고(마지막 2026-08-23), `live2d-web`을 6개 소스 파일에서 실제로
쓴다. 멈춘 것이 아니라 **`^0.3.1`에 묶여 있다.**

문제는 활성 소비자가 없다는 것이 아니라 **하나뿐인 활성 소비자가 네 버전
뒤에 있다**는 것이다. 그리고 그 소비자가 쓰는 공개 API는 `resolveAsset`
15회, `Live2DCanvas` 6회, `Live2DModel` 5회다. **0.5~0.7이 낸 기능(트래킹,
립싱크 helper, 모션 시퀀스, inspect, devtools, 접근성) 중 실제로 쓰이는 것은
0개다.** 이 수치가 아래 0.8 착수 조건의 근거다.

VS Code 확장은 보류, 히비엔(AI 캐릭터)은 재개 시 렌더링 층을 이 라이브러리로
바꾼다.
...
| --- | --- | --- | --- |
| 외부 의존 프로젝트 | 1 (Livesona, 활성이나 `^0.3.1` 고정) | 최신 minor를 통합한 활성 프로젝트 1 | 10 |
...
아래 상세는 260줄이라 무엇이 남았는지 한 번에 보이지 않는다. **"기다리는 것"
열이 실제 착수 가능 여부다.**

| 축 | 한 것 | 남은 것 | 기다리는 것 |
| --- | --- | --- | --- |
| A. 웹캠 트래킹 | 0.5: MediaPipe 서브패스, Perfect Sync 자동 매핑, 보정·평활화·loss 복귀, 감도 | Perfect Sync 실모델 체감 | 실모델·실기기 |
| B. 립싱크 | 0.4: 볼륨 helper | MotionSync 통합, wLipSync 기본 프로파일 | 소비자 요구 + Core 라이선스 경로 |
| C. 모션·표정 | 0.4: 시퀀스, 페이드 오버라이드, 가중 Idle, 상세 종료 상태 | 표정 가중 블렌드 | 감정 표현 소비자 |
| D. 다중 모델 | 없음 | Stage당 모델 N개, 컨텍스트 공유 | 실제 병목 |
| E. 성능·안정성 | 0.6: 추론 Worker, 번들 예산, 3단계 soak | MediaPipe 고도화(adaptive 해상도·FPS, 발열·배터리), GPU delegate, OffscreenCanvas, 모바일 실측, Firefox 14배 지연 | iOS 실측 |
| **E. 실카메라 게이트** | 없음 | 실제 카메라 녹화 픽스처로 부호·유지·loss 경계 자동 검사 | **아무것도** |
| **F. 개발자 경험** | 0.6: 3개국어 문서 사이트, 예제 4종, inspect, devtools | **10분 온보딩 테스트**, 외부 기여 유입 | **아무것도** (Vue/Svelte만 요구 대기) |
| **G. 신뢰·호환성** | 0.7: API 계약 스냅샷, 공개 계약 변경 규칙 | **지원 매트릭스**(VTube Studio 출력, 리거 도구별, Cubism 3), Core 버전 정책 | **아무것도** |

**굵게 표시한 셋은 소비자를 기다리지 않는다.** 이 문서가 "공개 계약·보안·
회귀처럼 측정된 위험을 줄이는 상시 품질 작업은 소비자 요청을 기다리지 않는다"고
둔 예외에 해당한다. 나머지는 전부 실측이나 요구를 기다리는 **대기 목록이지 할
일 목록이 아니다.**

G의 지원 매트릭스가 특히 눈에 띈다. 지금 검증은 공식 Hiyori 한 종뿐이라
**"내 모델이 되나"에 답하지 못한다.** 그것이 기반 라이브러리에 오는 첫 질문인데,
답이 없으면 소비자가 늘 이유도 없다.
...
- 번들 예산 단언: react·tracking entry의 raw/gzip 상한을 `verify-package`에
  적용. **구현·실측 통과**
- **실카메라 게이트**: 짧은 실제 카메라 녹화를 픽스처로 넣어 각도의 부호,
  자세 유지, loss 경계를 자동으로 잡는다. **이 축에서 소비자를 기다리지 않는
  유일한 항목이다.** 0.5가 실카메라로 결함 넷을 한꺼번에 드러냈고 이 문서가
  그것을 "가장 큰 교훈"으로 적었지만, 게이트는 아직 정지 초상 한 장이다.
  교훈만 기록되고 검사는 그대로면 트래킹을 다음에 건드릴 때 같은 방식으로
  깨진다. **미착수**
...
- Worker 오프로드 검토(물리·모션·`OffscreenCanvas`): 추론 Worker 뒤에
  측정으로 이득이 확인될 때만. **측정 대기**
- Firefox 추론은 한 번에 ~193ms로 Chromium의 14배다. Worker가 렌더 스레드를
  지켜 주지만 초당 5회가 상한이라 실사용에 못 미친다. 원인은 미조사이고,
  Firefox를 쓰는 소비자가 나오기 전에는 우선하지 않는다. **미조사·비차단**
- Perfect Sync 52개 매핑은 합성 픽스처로만 검증했다. 실제 Perfect Sync 모델로
  돌려본 적이 없어 체감은 미지수다. **실모델 대기**
...
- 문서 사이트: 타입에서 자동 생성한 레퍼런스 + 가이드 + 라이브 예제. 데모
  사이트(`live2d-web.heonys.dev`)를 흡수한다
...
| **0.7 안정화 후보** | API 계약 기준선, iOS Safari 실기기 검증, 독립 프로젝트 온보딩, 접근성·호환성·오류 안내, 측정 기반 MediaPipe 초기화 조정 | 공개 export·Canvas·오류·lifecycle snapshot, **iOS 결과표**, out-of-tree 소비자 온보딩 기록과 **발견된 결함 수정**. Android와 외부 프로젝트 수 자체는 발행 차단 조건이 아님. <br>_상태(2026-08-28): iOS 실기기 검증 통과. 결과표는 `compatibility.md`. 검증 과정에서 나온 결함(파라미터 중립, 프리뷰 크롭, 자산 배포)은 수정했고, pitch 응답은 관측 수단을 먼저 넣었다._ |
| **0.8** | 기능이 아니라 **소비자 복귀**. Livesona를 `^0.3.1`에서 최신으로 올리고 거기서 나온 결함으로 범위를 정한다. **G 지원 매트릭스를 병행**한다(소비자를 기다리지 않으므로) | 아래 "0.8 착수 조건"을 참조. 마이그레이션에서 나온 결함 수정과 그 과정 기록, 그리고 공식 Hiyori 외 모델 계열의 지원·미검증 구분 |
| 0.9 이후 | 0.8이 만든 실제 요구로 갈린다. 아래 "0.9 갈림길" 참조 | 착수 조건과 소비자·측정 결과를 결정 기록에 남긴 뒤 범위 확정. **낼 것이 없으면 내지 않는다** |
...
**이 로드맵은 스스로 막혀 있다.** 아래 수요 기반 후보 넷은 전부 "소비자가
요구하면" 조건이고, experimental 셋(`/tracking/mediapipe`, `/inspect`,
`/devtools`)의 졸업 조건도 전부 "실제 소비자 1곳"이다. 그리고 "요구가 없는
후보는 버전 번호가 와도 착수하지 않는다"고 이 문서가 못 박았다. 규칙을 그대로
따르면 **0.8에 착수할 수 있는 항목이 하나도 없다.**

탈출은 기능이 아니라 소비자에서 나온다. **0.8의 첫 작업은 Livesona를
`^0.3.1`에서 최신으로 올리는 것이다.** 하나의 작업이 네 가지를 동시에 푼다.

1. 선행 지표(최신 minor를 통합한 활성 프로젝트)를 0에서 1로 만든다
2. experimental 셋의 "실제 소비자 1곳" 조건에 유일한 경로를 연다
3. 네 버전 점프(0.3.1 → 0.7)가 곧 실제 마이그레이션 실측이다. 임시 스캐폴드로
   하는 온보딩 확인보다 훨씬 강한 증거다
4. 수요 기반 후보 넷 중 무엇이 실제로 필요한지 알려준다. 지금은 넷 다 추측이다

**차단 조건이 아니라 순서다.** 마이그레이션이 오래 걸리면 그 자체가 제품
결함의 크기를 재는 측정이며, 거기서 나온 결함 수정이 0.8의 범위가 된다.

**병행할 것.** G의 지원 매트릭스는 소비자를 기다리지 않으므로 같이 진행한다.
현재 검증은 공식 Hiyori 한 종뿐이고, VTube Studio 내보내기나 다른 리거 도구
출력이 되는지 아무도 모른다. **"내 모델이 되나"는 기반 라이브러리에 오는 첫
질문이고, 거기에 답하지 못하면 소비자가 늘 이유가 없다.** 마이그레이션이 한
소비자를 깊게 보는 일이라면 매트릭스는 오지 않은 소비자의 진입 장벽을 낮추는
일이라 서로 겹치지 않는다.

**측정할 것.** 전환에 걸린 시간, 문서만으로 해결한 문제와 소스를 읽어야 했던
문제의 수, 깨진 공개 계약, 0.5~0.7 기능 중 실제로 채택한 것과 채택하지 않은
이유. 마지막 항목이 가장 중요하다. 세 릴리스가 만든 기능을 하나뿐인 소비자가
쓰지 않는다면 그 이유가 다음 방향이다.

### 0.9 갈림길

0.9의 범위는 지금 정할 수 없고, 정해서도 안 된다. 0.8이 답할 질문 하나로
갈린다. **"Livesona가 0.5~0.7 기능을 왜 쓰지 않는가."** 전환이 깨끗하게 끝나도
이 질문은 남는다. 버전만 올라가고 트래킹·inspect·devtools를 여전히 쓰지
않으면 experimental 셋의 졸업 조건("실제 소비자 1곳")은 충족되지 않는다.

**a. 필요가 없어서.** OBS 아바타라는 제품 방향에 그 기능들이 맞지 않는다는
뜻이다. 그러면 0.5~0.7이 소비자를 잘못 가정한 것이므로, 졸업이 아니라 **유지
또는 축소를 검토**한다. experimental은 날짜 없는 유예가 아니라고 이 문서가
이미 적었다. 동시에 소비자 표의 빈 칸(VS Code 확장, 히비엔)에서 그 기능이
실제로 필요한 곳을 찾는다. 아픈 답이지만 가장 값지다.

**b. 있는 줄 몰라서.** 발견성 문제이므로 F로 간다. 특히 아직 하지 않은 **"처음
보는 개발자가 문서만으로 10분 안에 캐릭터를 띄우는 테스트"**가 목표 3번을
재는 유일한 측정이다.

**c. 쓰고 싶은데 모자라서.** 그 부족분이 0.8의 범위가 되고, 0.9는 그다음
요구를 받는다. 여기까지 오면 소비자 주도 순서가 정상 작동하기 시작한다.

**d. 아무것도 나오지 않으면 0.9를 내지 않는다.** 버전 번호는 완성도 점수가
아니라 호환되는 변경 묶음의 경계다. 바꿀 것이 없으면 0.7에 머문다.
**"다음 버전을 채워야 한다"는 압박으로 후보를 고르는 것이 이 문서가 네 번
경계한 실패의 시작점이다.** 그때도 소비자를 기다리지 않는 항목(E 실카메라
게이트, F 온보딩 측정, G 매트릭스)은 상시 품질 작업으로 계속 진행한다.
...
| --- | --- | --- | --- |
| Livesona | **활성, `^0.3.1` 고정** (26커밋/30일, 2026-08-27 실측) | 지금 쓰는 것은 `resolveAsset`·`Live2DCanvas`·`Live2DModel`뿐. 0.5~0.7 기능은 미사용 | 0.7 전환이 곧 마이그레이션 실측. OBS 실측, 장시간 안정성, 립싱크 튜닝 |
