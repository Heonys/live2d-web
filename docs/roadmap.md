...
상태 기준일: **2026-08-22**. 현재 발행 버전은 `0.3.1`이다. 지나온 마일스톤
(M0~M5)은 `private/docs/roadmap.md`에 이력으로 남아 있고, 이 문서는 앞으로의
방향만 다룬다.
...
- 릴리스마다 lint·typecheck·unit·tarball 소비자 3종·브라우저 3종 e2e를
  통과한다. 게이트를 줄이지 않는다
...
**소비자**: Livesona(OBS 아바타, M1 마무리 중)가 1호다. VS Code 확장이
다음 후보이고, 히비엔(AI 캐릭터)은 재개 시 렌더링 층을 이 라이브러리로
바꾼다.

## 성장 지표

기능 완성이 아니라 아래 수치로 진행을 판단한다. 측정일을 함께 적는다.

| 지표 | 현재 (2026-08-22) | 0.5 시점 | 1.0 시점 |
| --- | --- | --- | --- |
| 외부 의존 프로젝트 | 1 (Livesona) | 3 | 10 |
| 주간 다운로드 | 0 | 100 | 1,000 |
| 첫 이슈 응답 | 해당 없음 | 3일 이내 | 2일 이내 |
| 깨진 공개 계약 | 0 | 0 | 0 |
| 릴리스 주기 | 불규칙 | 월 1회 이상 | 분기 1회 이상 |

주간 다운로드는 후행 지표다. 외부 의존 프로젝트 수가 선행 지표이며, 그것이
늘지 않으면 기능을 더 쌓아도 의미가 없다.
...
- **공식 MotionSync 통합.** Live2D가 royalty-free로 제공하는 Cubism SDK
  MotionSync Plugin for Web(CRI LipSync)을 `/features/motionsync` 서브패스로
  감싼다. MotionSync Core는 Cubism Core처럼 비동봉이며 사용자가 URL로
  공급한다. 프로파일 없이 모음 구분 품질을 얻는 길이다
- wLipSync 모드는 유지하되, 프로파일 없는 사용자를 위한 기본 프로파일
  생성 도구는 만들지 않는다(MotionSync가 그 자리를 대신한다)

**범위 밖.** TTS, 음성 인식, 오디오 재생 파이프라인.

**검증.** 같은 음성 파일로 세 모드(볼륨·wLipSync·MotionSync)의 입 파라미터
궤적을 기록해 비교하는 벤치마크. 장시간 마이크 입력에서 AudioContext
suspend 복구와 장치 변경 대응.
...
- 파라미터 보간 유틸(`tween(id, to, ms)`)을 driver 위에 제공
- 물리·포즈 설정 노출(중력 방향, 바람)
...
- 모바일 실기기 GPU 게이트: iOS Safari·Android Chrome에서 startup·frame
  벤치마크를 릴리스 게이트로 승격
- soak 테스트 상시화: 120분 Chromium soak를 주간 CI로
- Worker 오프로드 검토: 물리·모션 계산을 Worker로, `OffscreenCanvas`로
  렌더까지. 측정으로 이득이 확인될 때만 착수
...
한다. 지금 문서는 README 3종과 API 레퍼런스뿐이고 검색에도 약하다.
...
- 모델 호환성 매트릭스: Cubism 3·4·5, Pro/Free 에디터 출력, VTube Studio
  내보내기, 주요 리거 도구의 출력을 실물로 수집해 CI에 고정
- Core 버전 정책 문서화: 어느 Core와 어느 Framework가 짝인지, 업그레이드
  시 무엇이 깨지는지
- Firefox 워클렛 문제 추적(현재 wLipSync만 제외), 브라우저별 알려진 제한 표
- 공개 계약 변경 규칙: breaking은 메이저 전환에서만, deprecation은 한 버전
  이상 유예
...
소비자 요구와 의존 관계로 순서를 정했다. 각 버전은 **소비자 하나가 실제로
받아쓰는 것**을 완료 조건으로 한다.

| 버전 | 내용 | 완료 조건 |
| --- | --- | --- |
| 0.4 | C 모션·표정 품질 일부(페이드·시퀀스), B 볼륨 driver 헬퍼 | Livesona가 헬퍼를 받아쓰고 자체 구현을 제거 |
...
| 0.8 | E 모바일 게이트·soak 상시화·Worker 검토 | 모바일 벤치마크가 릴리스 게이트에 포함 |
| 0.9 | F 문서 사이트·검사기·기여 가이드 | 외부 기여자의 첫 PR 병합 |
| 1.0 | G 계약 동결 | 외부 의존 3개 이상, 1년간 breaking 0, 모바일 실측 통과 |

순서는 바뀔 수 있다. 바뀌는 기준은 하나다: **어떤 소비자가 지금 그것을
필요로 하는가.** 요구하는 소비자가 없는 항목은 순서가 와도 착수하지 않는다.
...
| --- | --- | --- | --- |
| Livesona | M1 마무리 중 | B 헬퍼, A 트래킹(선택), C 모션 품질 | OBS 실측, 장시간 안정성, 립싱크 튜닝 |
| VS Code 확장 | 보류(다음 후보) | A 트래킹, E 상주 안정성 | 에디터 환경 제약, 장시간 실측 |
| 히비엔 | 재개 시 | C 표정 블렌드, D 다중 모델, B MotionSync | 감정 표현 요구, TTS 오디오 연결 사례 |
| 외부 | 0 | 전부 | 호환성 이슈, 기여 |
