...
사이트, 빌드 가능한 예제 4종과 선택형 Devtools도 구현됐다. 0.6·0.7 리뷰에서
Worker timeout·cleanup, 모델 검사 경계와 zip 한도도 다시 보완했다.

**못 하는 것**: 손·전신 센서 입력, 표정 가중 블렌드, Stage당 모델 둘 이상,
모바일 실기기 검증, 공식 MotionSync 립싱크, 외부 사용자가 확인한 온보딩과 첫
외부 기여.
