...
상태 기준일: **2026-08-25**. 현재 발행 버전은 `0.5.0`이고 `develop`에는 0.6
Worker와 0.7 문서·검사 구현이 있다. 지나온 마일스톤은 `private/docs/roadmap.md`에 이력으로 남아
...
## 현재 위치 (0.5.0 발행, 0.6 모바일·0.7 외부 검증 대기)
...
선택형 MediaPipe Worker, `/inspect` 공개 API·URL/zip 검사기, 세 언어 문서
사이트와 빌드 가능한 예제 4종도 구현됐다.

**못 하는 것**: 손·전신 센서 입력, 표정 동시 가중 적용, Stage당 모델 둘 이상,
모바일 GPU 실측 게이트, 공식 MotionSync 립싱크, 외부 사용자가 확인한 10분
온보딩과 첫 외부 기여.
...
- 120분 soak와 실제 하드웨어·모바일 측정은 비용이 큰 주기 작업으로 분리하고,
  0.6에서 정기 게이트로 성숙시킨다
- 번들 크기는 문서에 적는 것으로 끝내지 않고 `verify-package`가 entry별
  예산을 단언한다(root 100 kB는 이미 단언, react·tracking entry는 0.6에서)
...
| 지표 | 현재 (2026-08-22) | 0.5 시점 | 1.0 성장 목표 |
| --- | --- | --- | --- |
...
늘지 않으면 기능을 더 쌓아도 의미가 없다. 외부 의존 3개는 1.0을 검토할 최소
출시 조건이고, 표의 10개는 1.0 시기의 성장 성공을 판단하는 목표다.
...
- **공식 MotionSync 통합.** Live2D가 royalty-free로 제공하는 Cubism SDK
  ...
  공급한다. 프로파일 없이 모음 구분 품질을 얻는 길이다. **소비자가 볼륨
  드라이버로 부족하다고 말하기 전에는 착수하지 않는다.** 라이선스·별도
  Core·번들 복잡도가 붙는데, 그 값을 치를 이유는 아직 측정되지 않았다
...
의존 프로젝트는 기능이 아니라 이 축에서 생긴다. 그래서 0.7에 둔다. README
3개 국어 400줄은 이미 유지비다: 08-24 편집에서 JA만 문장이 유실됐고 08-25
리뷰가 복원했다. 문서 사이트가 그 중복을 흡수해야 한다.
...
2026-08-25에 E·F를 D·MotionSync 앞으로 당긴 근거는 docs/README.md의 같은 날
결정에 있다.
단, 공개 계약·보안·회귀처럼 이미 측정된 위험을 줄이는 상시 품질 작업은 별도
소비자 요청을 기다리지 않는다.
